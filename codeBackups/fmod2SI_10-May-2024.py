...
#arrow(x_tail + 1, y_tail - .4, dx, dy, width=.1, length_includes_head=True, color="C2")
    ...
    axis.quiver(xq,yq,uq,vq, angles='xy',color=color)
...
for t in time:
    ...
    if state == GROWING:
        ...
        Lddot = max(0, (rReel_SIu**2/J) * ( 0.5 * P*area_m2 -Fdrag(L,Ldot) -Fcoulomb ) )
...
if PLOT_TYPE == 'OVERLAY':
    ...
    axs[1,0].plot(np.array(ed['time']), np.array(ed['P']), 'r--')  # Experimental Data
